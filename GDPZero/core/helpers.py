class DialogSession():
	...
	def to_string_rep(self, keep_sys_da=False, keep_user_da=False, max_turn_to_display=-1): ## histoy에서 이전turn을 제외하고 몇개의 history로 보여주는지.. 
		history = ""
		num_turns_to_truncate = 0
		if max_turn_to_display > 0:
			num_turns_to_truncate = max(0, len(self.history) // 2 - max_turn_to_display) # 히스토리 8, 멕스 턴 3 -> 1
		
		for i, (role, da, utt) in enumerate(self.history): # 8번대화 1 2 1 2 1 2 1 2 -> 4
			if (i // 2) < num_turns_to_truncate: # 2 // 2 < 1
				continue
			if i % 2 == 0: # 0 % 2 = 0 -> sys
