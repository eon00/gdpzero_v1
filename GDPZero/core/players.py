...
class P4GSystemPlanner(DialogPlanner):
	def __init__(self, 
			...
			conv_examples: List[DialogSession] = []) -> None:  # example history
		...
		return
	def process_exp(self, keep_sys_da=True, keep_user_da=False): # conversation example -> 각 turn마다 sys , [DialogSession1, DialogSession2...]
		prompt_exps = ""
		for exp in self.conv_examples: #exp = DialogSession1
	...
	def predict(self, state:DialogSession) -> "Tuple[np.ndarray, float]":
		...
		prob = np.zeros(len(self.dialog_acts)) # 각 dialog act 가 나온 횟수 저장 -> prob 
		prob += self.smoothing
		for da in sampled_das:
			prob[self.dialog_acts.index(da)] += 1
		prob /= prob.sum()
		v = self.heuristic(state) # user의 응답score평균 
		return prob, v
...
class PersuaderChatModel(PersuaderModel):
	def __init__(self,
		...
		self.prompt_examples = self.process_chat_exp()  ## 모든 발화문 담긴 [[], [], [], ...]에 대해 시작하는 propmt
		return

	def process_chat_exp(self):
		prompt_exps = []
		for exp in self.conv_examples: ## conv_example = [[exp1], [exp2], ...] = [[(role,da,utt), ...], [], [], ...]
			prompt_exps += self.__proccess_chat_exp(exp) # 지금 까지 나왔던 대화
			prompt_exps.append({
				"role":"system", "content": self.new_task_prompt # 이제 대화를 시작하는것이기 때문에 system에게 greeting하는 new_task_prompt 추가 
			})
		return prompt_exps[:-1]

	 ## 기존의 role, da, utt를 순서대로 저장하고 system이 다음의 대화를 하도록 prompt message를 만드는 단계 
	 ## -> max를 통해 하나의 대화문을 몇개부터 보여줄것인지 결정할수 있음 /
	## 시작하는 turn이 usr로 시작할경우  history보다 작으면 system은 마지막에 greeting하도록 prompt messages
	def __proccess_chat_exp(self, exp:DialogSession, max_hist_num_turns: int = -1):  ## ex) max 2
		if len(exp) == 0:
			return []
		...
		prompt_messages = []
		num_turns_to_truncate = 0
		if max_hist_num_turns > 0:  # ex) 2 > 0 
			num_turns_to_truncate = max(0, len(exp) // 2 - max_hist_num_turns) # 10//2 - 2 = 3
		
		
		next_sys_da = PersuasionGame.S_Greeting # 다음 sys의 전략 행동 = greeting 
		for i, (role, da, utt) in enumerate(exp):   
			# truncate to reduce the size of the prompt
			if (i // 2) < num_turns_to_truncate:   # ex) max=2인 경우 이부분에 의해서 6,7,8,9 즉, sys 2번 , usr2번의 발화문만 보임   
				continue
			if role == PersuasionGame.SYS:
				prompt_messages.append({
					"role": "assistant",
					"content": f"{role}: {utt}".strip()  # sys인 경우 prompt_messages 에 추가 
				})
			else:
				if i+1 < len(exp.history):  #################  histoy가 뭔지 잘 모르겠다.
					next_sys_da = exp[i+1][1]
					prompt_messages.append({
						"role": "user",
						"content": f"{role}: {utt}\n{self.da_prompts_mapping[next_sys_da]}".strip() # PersuasionGame.S_Greeting:"The Persuader greets the Persuadee.",
					})
				else:
					prompt_messages.append({
						"role": "user",
...
class PersuadeeChatModel(PersuadeeModel):
	...
	def __process_heuristics_chat_exp(self, dialog:DialogSession):
		...
		prompt_messages.append({
			"role": 'user',
			"content": prompt_q   # sys 발화, user 발화,... sys 발화,  usr 발화 / user의 기분: 
		})
		prompt_messages.append({
			"role": 'assistant',
			"content": f"{answer_da}"
		})
		return prompt_messages # prompt_messages = [ {'user': [sys 발화, user 발화,... sys 발화,  usr 발화,user의 기분: ]}, {'assistant':'answer_da = dialog[-1][1]'}]
	...
	def predict_da(self, state:DialogSession, never_end=True) -> str:
		...
		# insert prop to donate, and compute the likelihood of user simulator agreeing to donate
		assert(state[-1][0] == PersuasionGame.USR) # sys 이어야 함 ->  user의 반응을 아는것 

		messages = [
			{'role': 'system', 'content': self.task_prompt},
			*self.process_heurstics_chat_exp(new_task_prompt=self.new_task_prompt),
			{'role': 'system', 'content': self.new_task_prompt} # "The following is a new conversation between a Persuader and a Persuadee (you). You may or may not want to donate to Save the Children."
		]
		new_dialog_session = self.__truncate_heuristics_dialog(state, -1)   # 전체 대화 history가 아니라 앞부분을 생략할 수 있음 
		messages += self.__process_heuristics_chat_exp(new_dialog_session)[:-1] # prompt_messages = [ {'user': [sys 발화, user 발화,... sys 발화,  usr 발화,user의 기분: ]}, {'assistant':'answer_da = dialog[-1][1]'}]

		# majority vote, same as value function
		inf_args = {
			"max_new_tokens": 5, # 새로 생성할 토큰의 최대 수
			"temperature": 0.7, # 생성된 텍스트의 다양성을 조절하는 파라미터
			"return_full_text": False, # 전체 텍스트를 반환할지 여부
			"do_sample": True, # 샘플링을 수행할지 여부
			"num_return_sequences": 5, # # 반환할 시퀀스의 수
		}
		datas = self.backbone_model.chat_generate(messages, **inf_args) # 응답한 txt들 
		# process into das  # 생성된 응답을 대화 행위(DA)로 변환
		sampled_das: list = []
		for resp in datas:
